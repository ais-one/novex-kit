...
Give the app its own `tsconfig.json` — this is the one actually used (and verified against real strict-mode output, not just written and assumed to work) in `apps/vision-common` and `apps/vision-queue-consumer`:
...
Two things strict mode surfaces that `strict: false` hides, both hit while building the reference example in `apps/vision-common`/`apps/vision-queue-consumer`:
...
- `apps/vision-queue-consumer` — real controller(consumer)/service/repository split for a message-driven (non-HTTP) app; its `tsconfig.json`/`global.d.ts` are the copy-paste strict-mode template referenced above.
