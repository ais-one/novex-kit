...
import { iamUsers as table } from '@db/iam/schema';
...
const update = async (req, res) => {
  const result = await db().update(table).set(req.body).where(eq(table.id, req.params.id));
...
const remove = async (req, res) => {
  const result = await db().delete(table).where(eq(table.id, req.params.id));
  const count = result.rowCount ?? 0;
  return res.status(count ? 200 : 404).json({ count });
};
